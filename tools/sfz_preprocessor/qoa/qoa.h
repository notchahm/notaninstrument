...
#ifndef QOA_SLICES_PER_FRAME
// Guarded (upstream doesn't guard this) so a build can override it via
// -DQOA_SLICES_PER_FRAME=<n> -- notaninstrument's own encoder CLI
// (tools/sfz_preprocessor/qoa/qoa_encode_cli.c) does exactly that, using
// a much smaller frame than the 256-slice (160ms @ 32kHz) stock default
// so loop points can land close to a frame boundary (see sfz_to_nib.py's
// find_best_loop_points) without eating a large chunk of a short loop.
